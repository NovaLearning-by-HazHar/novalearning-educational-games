/**
 * Mzansi Journey — Audio synthesis. Province discovery sounds + celebration.
...
/** Province discovery chime — warm ascending two-note */
export async function generateProvinceChime(): Promise<Blob> {
  const duration = 0.5;
  const ctx = new OfflineAudioContext(1, Math.ceil(SAMPLE_RATE * duration), SAMPLE_RATE);
...
/** Map tap — gentle click */
export async function generateMapTap(): Promise<Blob> {
  return generateTone({
    frequency: 500,
    duration: 0.1,
    type: 'triangle',
...
/** Sticker collect — bright sparkle */
export async function generateStickerCollect(): Promise<Blob> {
  const duration = 0.4;
  const ctx = new OfflineAudioContext(1, Math.ceil(SAMPLE_RATE * duration), SAMPLE_RATE);
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    gain.gain.linearRampToValueAtTime(0.0, start + 0.12);
