/**
 * Ubuntu Garden — Audio synthesis. Placement + task + celebration sounds.
...
/** Plant sound — earthy soft thud + sparkle */
export async function generatePlantSound(): Promise<Blob> {
  const duration = 0.4;
  const ctx = new OfflineAudioContext(1, Math.ceil(SAMPLE_RATE * duration), SAMPLE_RATE);
  ...
  gain1.gain.linearRampToValueAtTime(0.4, 0.01);
  gain1.gain.linearRampToValueAtTime(0, 0.15);
  osc1.connect(gain1);
  ...
  gain2.gain.linearRampToValueAtTime(0.3, 0.12);
  gain2.gain.linearRampToValueAtTime(0, 0.35);
  osc2.connect(gain2);
...
/** Task complete — bright ascending ding */
...
/** Item tap — gentle click */
export async function generateItemTap(): Promise<Blob> {
  return generateTone({
    frequency: 600,
    duration: 0.1,
    type: 'triangle',
...
/** Ambient garden — gentle nature hum */
export async function generateAmbientGarden(): Promise<Blob> {
  const duration = 3;
  const ctx = new OfflineAudioContext(1, Math.ceil(SAMPLE_RATE * duration), SAMPLE_RATE);
